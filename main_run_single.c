...
void print_complex_to_csv(char * filename, double complex * array, int N)
{ 
    FILE *fp;
...
void print_real_to_csv(char * filename, double complex * array, int N)
{
    FILE *fp;
    fp = fopen(filename, "w");
...
{
    // Test parameters
    int nsamples_array[] = {256}; // # samples per iteration (need to be powers of two)
    int n_iterations = 1;
    float ang_freq = 50;
    ...
    for (int i = 0; i < array_size; i++)
    {
        ...
        for (int j = 0; j < n_iterations; j++)
        {
            ...
            print_real_to_csv("./DFT/real.csv", time_signal, N); 
            print_complex_to_csv("./DFT/complex.csv", new_signal, N);   
            free(new_signal);
        }
        ...
        for (int j = 0; j < n_iterations; j++)
        {
            ...
            print_real_to_csv("./FFT_out/real.csv", time_signal, N); 
            print_complex_to_csv("./FFT_out/complex.csv", new_signal, N); 
        ...
        for (int j = 0; j < n_iterations; j++)
        {
            print_real_to_csv("./FFT_in/real.csv", time_signal, N); 
            clock_t begin = clock();
            FFT_inplace(time_signal, N);
            clock_t end = clock();
            FFT2_avg_time += (double)(end - begin);
            print_complex_to_csv("./FFT_in/complex.csv", time_signal, N); 
        }
